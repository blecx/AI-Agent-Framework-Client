...
function isActionable(check: ReadinessCheck) {
  return (
    (check.status === 'warn' || check.status === 'fail' || check.status === 'notAssessed') &&
    !!check.actionKey &&
    !!check.actionUrl
  );
}
...
export default function ReadinessPanel({ readiness }: ReadinessPanelProps) {
  ...
  return (
    <section className={`readiness-panel readiness-panel--${readiness.overallStatus}`}>
      <div className="readiness-panel-status">
        <span className="readiness-panel-icon" aria-hidden="true">
          {ICONS[readiness.overallStatus]}
        </span>
        <span className="readiness-panel-label">{t(`rd.state.${readiness.overallStatus}`)}</span>
      </div>

      <div className="readiness-panel-summary">
        <span>{t('rd.summary.passed', { count: readiness.summary.passed })}</span>
        <span>{t('rd.summary.warnings', { count: readiness.summary.warnings })}</span>
        <span>{t('rd.summary.failed', { count: readiness.summary.failed })}</span>
      </div>
          ...
          <ul>
            {nextActions.map((check) => (
              <li key={check.id}>
                ...
                >
                  {check.actionKey ? t(`rd.actions.${check.actionKey}`) : t('rd.actions.open')}
                </button>
              </li>
            ))}
          </ul>
