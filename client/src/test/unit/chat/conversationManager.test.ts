...
describe('conversationManager', () => {
  ...
  describe('processConversationResponse', () => {
    it('should process title response correctly', () => {
      const initialState = initCreateRAIDConversation(
        'PROJ-001',
        RAIDType.RISK,
      );
      ...
      expect((result.state as typeof initialState)?.collectedData.title).toBe(
        'Security vulnerability in API',
      );
    ...
    it('should parse RAID type from natural language', () => {
      ...
      expect((result.state as CreateRAIDState)?.collectedData.type).toBe(
        RAIDType.RISK,
      );
      expect(result.isComplete).toBe(false);
    });
    ...
    it('should parse priority from natural language', () => {
      ...
      state = processConversationResponse(state, 'Test Title')
        .state as CreateRAIDState;
      state = processConversationResponse(state, 'Test Description')
        .state as CreateRAIDState;
      expect(state.steps[state.currentStep].field).toBe('priority');

      const result = processConversationResponse(state, 'This is critical');

      expect((result.state as CreateRAIDState)?.collectedData.priority).toBe(
        RAIDPriority.CRITICAL,
      );
    });
    ...
    it('should mark conversation as complete after all steps', () => {
      ...
      state = processConversationResponse(state, 'Test Description')
        .state as CreateRAIDState;
      state = processConversationResponse(state, 'high')
        .state as CreateRAIDState;
      const result = processConversationResponse(state, 'John Doe');

      expect(result.isComplete).toBe(true);
      expect(result.nextPrompt).toBeNull();
      expect((result.state as CreateRAIDState)?.collectedData.owner).toBe(
        'John Doe',
      );
    });
  ...
  describe('getConversationProgress', () => {
    ...
    it('should show 100% when conversation is complete', () => {
      ...
      state = processConversationResponse(state, 'high')
        .state as CreateRAIDState;
      state = processConversationResponse(state, 'John Doe')
        .state as CreateRAIDState;
  ...
  describe('field value parsing', () => {
    it('should parse various RAID type inputs', () => {
      ...
      expect((riskResult.state as CreateRAIDState)?.collectedData.type).toBe(
        RAIDType.RISK,
      );

      const assumptionResult = processConversationResponse(state, 'assumption');
      expect(
        (assumptionResult.state as CreateRAIDState)?.collectedData.type,
      ).toBe(RAIDType.ASSUMPTION);

      const issueResult = processConversationResponse(state, 'issue');
      expect((issueResult.state as CreateRAIDState)?.collectedData.type).toBe(
        RAIDType.ISSUE,
      );

      const depResult = processConversationResponse(state, 'dependency');
      expect((depResult.state as CreateRAIDState)?.collectedData.type).toBe(
        RAIDType.DEPENDENCY,
      );
    });
    ...
    it('should parse various priority inputs', () => {
      let state = initCreateRAIDConversation('PROJ-001', RAIDType.RISK);
      state = processConversationResponse(state, 'Title')
        .state as CreateRAIDState;
      state = processConversationResponse(state, 'Description')
        .state as CreateRAIDState;

      const critResult = processConversationResponse(state, 'critical');
      expect(
        (critResult.state as CreateRAIDState)?.collectedData.priority,
      ).toBe(RAIDPriority.CRITICAL);
      ...
      expect(
        (highResult.state as CreateRAIDState)?.collectedData.priority,
      ).toBe(RAIDPriority.HIGH);
    });
    ...
    it('should trim whitespace from text fields', () => {
      ...
      expect((result.state as CreateRAIDState)?.collectedData.title).toBe(
        'Test Title',
      );
    });
  });
